...
with a warning shown before the restore, every `imageUrl` is constrained to `https:`,
an `image/*` data URI, or a filename-only `private-exercise:` marker resolved through
the authenticated sync server, and every exercise `videoUrl` is constrained to a plain `https:`
