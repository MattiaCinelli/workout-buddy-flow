...
const CourseDetail = () => {
  ...
  const handleCompleteWorkout = async (workoutId: string) => {
    await completeWorkoutInCourse(course.id, workoutId);
      ...
      <main className="flex-1 container mx-auto py-6 px-4 md:px-6">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm text-muted-foreground">
                {sortedWorkouts.filter(w => w.completed).length}/{sortedWorkouts.length} workouts
              </span>
            </div>
        ...
        <div className="space-y-3">
          {sortedWorkouts.map((courseWorkout, index) => {
            const workout = getWorkoutById(courseWorkout.workoutId);
            const isNext = nextWorkout?.workoutId === courseWorkout.workoutId;
                    ...
                    <div className="flex gap-2">
                      {courseWorkout.completed ? (
                        ...
                        <>
                          ...
                          <Button 
                            size="sm"
                            onClick={() => handleCompleteWorkout(courseWorkout.workoutId)}
                          >
