...
const ExerciseForm: React.FC<ExerciseFormProps> = ({
  ...
  const handleFormSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    onSubmit({
      ...
      collectionOrder: values.collectionName?.trim() ? toNumber(values.collectionOrder) : undefined,
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-4">
        ...
        <div className="grid gap-3 rounded-lg border p-3 sm:grid-cols-2">
          <div><label className="text-sm font-medium" htmlFor="collection-name">Exercise collection</label><Input id="collection-name" {...form.register('collectionName')} placeholder="e.g. Push-up progression" /><p className="mt-1 text-xs text-muted-foreground">Use the same collection name for related exercise cards.</p></div>
          <div><label className="text-sm font-medium" htmlFor="collection-order">Level / position</label><Input id="collection-order" type="number" min={1} {...form.register('collectionOrder')} placeholder="1" /></div>
        </div>
